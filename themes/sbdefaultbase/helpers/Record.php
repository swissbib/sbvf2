...
class VuFind_Theme_Root_Helper_Record extends Zend_View_Helper_Abstract
{
     ...
     *
     */
    public function getFrbrLink(){

        $frbrNr = $this->driver->getFrbrNr();

        $link = "";

        if (!empty($frbrNr)) {

            //konstruiere einen link jetzt geht es darum, besser zu verstehen, wie ich mit anderen Werten umgehen kann
    ...
    /**
     * Get the Label for a coded format
     *
     * @param $mediatype code for the mediatype to be converted into the label
     *
     * @return string
     */
    public function getFormatName($mediatype)
    {

        //todo: ein vernuenftiger Algorithmus um die mediatypewerte in labels umwndeln zu koennen (Sprache beruecksictigen)
        return "Buch";
    ...
    public function getFormatClass($format)
    {
        ...
        //todo: ein vernuenftiger Algorithmus um die mediatywerte auslesen zu koennen
        return "icon_format_book";
